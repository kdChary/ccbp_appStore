...
class AppStore extends Component {
  state = {category: tabsList[0].tabId, userInput: ''}
  ...
  render() {
    const {category, userInput} = this.state
    const filterApps = appsList.filter(eachApp => eachApp.category === category)
    const userSearchResults = appsList.filter(
      eachApp =>
        eachApp.appName.toLowerCase().includes(userInput.toLowerCase()) &&
        eachApp.category === category,
    )
    const filteredAppList = userInput.length ? userSearchResults : filterApps
          ...
          <ul className="tab-item-list">
            {tabsList.map(eachItem => (
              <TabItem
                ...
                isIdSame={eachItem.tabId === category}
                key={eachItem.tabId}
              />
            ))}
          </ul>
